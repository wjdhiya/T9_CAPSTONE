<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('pengmas.create') }}" class="px-4 py-2 text-white rounded-lg bg-gray-800 hover:bg-gray-700 transition-colors shadow-sm">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex justify-center items-center space-x-3">
                                        <a href="{{ route('pengmas.show', $item) }}" class="text-indigo-600 hover:text-indigo-900 tooltip" title="Lihat Detail">
                                            <i class="fas fa-eye text-lg"></i>
                                        </a>
                                        
                                        @if (auth()->user()->isDosen() && $item->user_id === auth()->id() && $item->status_verifikasi !== 'verified')
                                            <a href="{{ route('pengmas.edit', $item) }}" class="text-blue-600 hover:text-blue-900" title="Edit">
                                                <i class="fas fa-edit text-lg"></i>
                                            </a>
                                            
                                            <form action="{{ route('pengmas.destroy', $item) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini? Data yang dihapus tidak dapat dikembalikan.');">
                                                @csrf
                                                @method('DELETE')
