            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <a href="{{ route('penelitian.index') }}" class="relative flex flex-col sm:flex-row items-center p-4 rounded-lg bg-red-50 hover:bg-red-100 transition-colors border border-red-200">
                                ...
                                @if(($isKaprodi ?? false) && (($kaprodiNotifications['penelitian'] ?? 0) > 0))
                                    <span class="absolute -top-2 -right-2 min-w-[1.25rem] h-5 px-1 rounded-full bg-red-600 text-white text-xs font-bold flex items-center justify-center">
                                        {{ $kaprodiNotifications['penelitian'] }}
                                    </span>
                                @endif
                            </a>
                            @if(auth()->user()->canInputTriDharma())
                                <a href="{{ route('penelitian.create') }}" class="flex flex-col sm:flex-row items-center p-4 rounded-lg bg-white hover:bg-red-50 transition-colors border border-red-200">
                                    <svg class="w-6 h-6 mb-2 sm:mb-0 sm:mr-3 text-red-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if($isKaprodi ?? false)
                            <div class="mt-4 border-t pt-4">
                                <form id="penelitianImportForm" method="POST" action="{{ route('imports.tridharma', ['type' => 'penelitian']) }}" enctype="multipart/form-data">
                                    @csrf
                                    <div id="penelitianDropZone" class="relative w-full h-32 border-2 border-gray-300 border-dashed rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors group cursor-pointer">
                                        
                                        <input type="file" id="penelitianImportFile" name="file" accept=".csv, .xlsx, .xls"
                                            class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                                        
                                        <div id="penelitianContent" class="flex flex-col items-center justify-center h-full pt-5 pb-6 text-center pointer-events-none">
                                            <i class="fas fa-file-upload text-gray-400 text-2xl mb-2 group-hover:text-blue-500 transition-colors"></i>
                                            <p class="text-sm text-gray-500"><span class="font-semibold text-blue-600">Klik</span> atau drag file CSV/XLSX</p>
                                        </div>

                                        <div id="penelitianPreview" class="hidden absolute inset-0 w-full h-full bg-green-50 rounded-lg flex flex-col items-center justify-center z-20">
                                            <i class="fas fa-file-csv text-green-600 text-2xl mb-1"></i>
                                            <p id="penelitianFilename" class="text-sm font-medium text-gray-900 truncate w-3/4 text-center px-2"></p>
                                            <button type="button" id="penelitianRemoveBtn" class="mt-2 text-xs text-red-600 hover:text-red-800 underline font-semibold cursor-pointer relative z-30">
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <a href="{{ route('publikasi.index') }}" class="relative flex flex-col sm:flex-row items-center p-4 rounded-lg bg-green-50 hover:bg-green-100 transition-colors border border-green-200">
                            ...
                            @if(auth()->user()->canInputTriDharma())
                                <a href="{{ route('publikasi.create') }}" class="flex flex-col sm:flex-row items-center p-4 rounded-lg bg-white hover:bg-green-50 transition-colors border border-green-200">
                                    <svg class="w-6 h-6 mb-2 sm:mb-0 sm:mr-3 text-green-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                    </svg>
                        ...
                        @if($isKaprodi ?? false)
                            <div class="mt-4 border-t pt-4">
                                <form id="publikasiImportForm" method="POST" action="{{ route('imports.tridharma', ['type' => 'publikasi']) }}" enctype="multipart/form-data">
                                    @csrf
                                    <div id="publikasiDropZone" class="relative w-full h-32 border-2 border-gray-300 border-dashed rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors group cursor-pointer">
                                        <input type="file" id="publikasiImportFile" name="file" accept=".csv, .xlsx, .xls"
                                            class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                                        <div id="publikasiContent" class="flex flex-col items-center justify-center h-full pt-5 pb-6 text-center pointer-events-none">
                                            <i class="fas fa-file-upload text-gray-400 text-2xl mb-2 group-hover:text-blue-500 transition-colors"></i>
                                            <p class="text-sm text-gray-500"><span class="font-semibold text-blue-600">Klik</span> atau drag file CSV/XLSX</p>
                                        </div>
                                        <div id="publikasiPreview" class="hidden absolute inset-0 w-full h-full bg-green-50 rounded-lg flex flex-col items-center justify-center z-20">
                                            <i class="fas fa-file-csv text-green-600 text-2xl mb-1"></i>
                                            <p id="publikasiFilename" class="text-sm font-medium text-gray-900 truncate w-3/4 text-center px-2"></p>
                                            <button type="button" id="publikasiRemoveBtn" class="mt-2 text-xs text-red-600 hover:text-red-800 underline font-semibold cursor-pointer relative z-30">
                                                <i class="fas fa-times mr-1"></i> Ganti File
                                            </button>
                                        </div>
                                    </div>
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <a href="{{ route('pengmas.index') }}" class="relative flex flex-col sm:flex-row items-center p-4 rounded-lg bg-blue-50 hover:bg-blue-100 transition-colors border border-blue-200">
                            ...
                            @if(auth()->user()->canInputTriDharma())
                                <a href="{{ route('pengmas.create') }}" class="flex flex-col sm:flex-row items-center p-4 rounded-lg bg-white hover:bg-blue-50 transition-colors border border-blue-200">
                                    <svg class="w-6 h-6 mb-2 sm:mb-0 sm:mr-3" style="color: #003366;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                    </svg>
                        ...
                        @if($isKaprodi ?? false)
                            <div class="mt-4 border-t pt-4">
                                <form id="pengmasImportForm" method="POST" action="{{ route('imports.tridharma', ['type' => 'pengmas']) }}" enctype="multipart/form-data">
                                    @csrf
                                    <div id="pengmasDropZone" class="relative w-full h-32 border-2 border-gray-300 border-dashed rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors group cursor-pointer">
                                        <input type="file" id="pengmasImportFile" name="file" accept=".csv, .xlsx, .xls"
                                            class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                                        <div id="pengmasContent" class="flex flex-col items-center justify-center h-full pt-5 pb-6 text-center pointer-events-none">
                                            <i class="fas fa-file-upload text-gray-400 text-2xl mb-2 group-hover:text-blue-500 transition-colors"></i>
                                            <p class="text-sm text-gray-500"><span class="font-semibold text-blue-600">Klik</span> atau drag file CSV/XLSX</p>
                                        </div>
                                        <div id="pengmasPreview" class="hidden absolute inset-0 w-full h-full bg-green-50 rounded-lg flex flex-col items-center justify-center z-20">
                                            <i class="fas fa-file-csv text-green-600 text-2xl mb-1"></i>
                                            <p id="pengmasFilename" class="text-sm font-medium text-gray-900 truncate w-3/4 text-center px-2"></p>
                                            <button type="button" id="pengmasRemoveBtn" class="mt-2 text-xs text-red-600 hover:text-red-800 underline font-semibold cursor-pointer relative z-30">
                                                <i class="fas fa-times mr-1"></i> Ganti File
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <div class="space-y-4">
                            <a href="{{ route('penelitian.index') }}" class="block p-4 border rounded-lg hover:bg-gray-50 transition">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="px-3 py-1 bg-red-100 text-red-800 rounded-full text-sm font-medium">{{ $verificationQueue['penelitian'] ?? 0 }} Menunggu</span>
                                </div>
                            </a>
                            <a href="{{ route('publikasi.index') }}" class="block p-4 border rounded-lg hover:bg-gray-50 transition">
                                <div class="flex justify-between items-center">
                                    <span>Publikasi</span>
                                    <span class="px-3 py-1 bg-red-100 text-red-800 rounded-full text-sm font-medium">{{ $verificationQueue['publikasi'] ?? 0 }} Menunggu</span>
                                </div>
                            </a>
                            <a href="{{ route('pengmas.index') }}" class="block p-4 border rounded-lg hover:bg-gray-50 transition">
                                <div class="flex justify-between items-center">
                                    <span>Pengabdian</span>
                                    <span class="px-3 py-1 bg-red-100 text-red-800 rounded-full text-sm font-medium">{{ $verificationQueue['pengmas'] ?? 0 }} Menunggu</span>
                                </div>
                            </a>
                        </div>
                    </div>
                                        ...
                                        <div class="flex items-center justify-between p-2 rounded-lg ${idx === 0 ? 'bg-yellow-50 border border-yellow-200' : 'bg-gray-50'}">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-xs text-gray-500">NIDN: ${l.nidn || '-'}</p>
                                                </div>
                                            </div>
